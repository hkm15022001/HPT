...
func GetEmployeeListHandler(c *gin.Context) {
	...
	store.GetReadOnlyConnection().Table("employees as e").Select(selectPart).Joins(leftJoin1).Joins(leftJoin2).
...
func GetEmployeeHandler(c *gin.Context) {
	...
	leftJoin1 := "left join employee_types as et on e.employee_type_id = et.id"
	leftJoin2 := "left join delivery_locations as dl on e.delivery_location_id = dl.id"
	if err := store.GetReadOnlyConnection().Table("employees as e").Select(selectPart).Joins(leftJoin1).Joins(leftJoin2).
		Where(" e.deleted_at is NULL ").
...
func CreateEmployeeFormData(c *gin.Context) {
	...
	store.GetReadOnlyConnection().Table("employee_types as et").Select(selectPart).Order("et.id asc").Find(&employeeTypeOptions)
	...
	store.GetReadOnlyConnection().Table("delivery_locations as dl").Select(selectPart).Order("dl.id asc").Find(&deliveryLocationOptions)
...
func CreateEmployeeHandler(c *gin.Context) {
	...
	if err := store.GetReadWriteConnection().Create(employee).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func UpdateEmployeeFormData(c *gin.Context) {
	...
	if err := store.GetReadOnlyConnection().First(employee, c.Param("id")).Error; err != nil {
		c.AbortWithStatus(http.StatusNotFound)
		return
	}
...
func UpdateEmployeeHandler(c *gin.Context) {
	...
	if err := store.GetReadWriteConnection().Model(&employee).Updates(&employee).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func DeleteEmployeeHandler(c *gin.Context) {
	...
	if err := store.GetReadWriteConnection().Delete(&model.UserAuthenticate{}, employee.UserAuthID).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	if err := store.GetReadWriteConnection().Delete(&model.Employee{}, c.Param("id")).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
